...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # 1x1 conv on layer 7 and upsample 4x
    l7_1x1conv = tf.layers.conv2d(vgg_layer7_out, \
                                  num_classes, kernel_size=1, strides=1)
                                  
    l4_1x1conv = tf.layers.conv2d(vgg_layer4_out, \
                                  num_classes, kernel_size=1, strides=1)
                                  
    l3_1x1conv = tf.layers.conv2d(vgg_layer3_out, \
                                  num_classes, kernel_size=1, strides=1)                                  
    
    ######
    l7_1x1conv_x2=tf.layers.conv2d_transpose(l7_1x1conv, num_classes, \
                                    kernel_size=8, strides=2, padding='same')
                                     
    o1 = tf.add(l7_1x1conv_x2, l4_1x1conv)
   
    ######
    o2 = tf.layers.conv2d_transpose(o1, num_classes, \
                                    kernel_size=8, strides=2, padding='same')
                                  
    o3 = tf.add(l3_1x1conv, o2)
    
                                  
    '''   
    l4_x1=tf.layers.conv2d_transpose(vgg_layer4_out, \
                                     num_classes, 1, strides=1, padding='same')

    l3_x1=tf.layers.conv2d_transpose(vgg_layer3_out, \
                                     num_classes, 1, strides=1, padding='same')


    o1 = tf.add(l7_x2, l4_x1)

    o2 = tf.layers.conv2d_transpose(o1, \
                                    num_classes, 32, strides=2, padding='same')

    o3 = tf.add(o2, l3_x1)
    '''

    # Upsample the layer by 8x to create fcn8
    output =tf.layers.conv2d_transpose(o3, num_classes, \
